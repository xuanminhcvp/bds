...
class TransactionService:
    ...
    async def get_transaction_by_id(db: AsyncSession, transaction_id: int):
        return await TransactionRepository.get_transaction_by_id(db, transaction_id)
    
    @staticmethod
    async def create_transaction(db: AsyncSession, transaction_data: TransactionCreate):
        property = await PropertyService.get_property_by_id(db, transaction_data.property_id)

        if not property or property.status != "avaiable":
            return {"success": False, "message": "Property is not avaiable"}

        transaction = await TransactionRepository.create_transaction(db, transaction_data)
